...
function StackTitle({ title }: { title: string }) {
  return (
    <motion.div
      className="my-2 flex items-center justify-between gap-2 md:my-6 md:gap-4"
...
function SkillName({ title, index }: { title: string; index?: number }) {
  return (
    <motion.div
      ...
      className="flex items-center px-6 py-2 md:px-10"
    >
      <Heading
        as="h3"
        className="text-muted-foreground hover:text-primary font-bold tracking-tight transition-all duration-300 hover:scale-105"
      >
        {title}
      </Heading>
    </motion.div>
